...
| 2026-04-28 | Eval | **A/B evaluation complete.** 4 arms (T=knowledge graph, R=naive RAG, P=hierarchical docs, B=no memory), 12 tasks x 10 runs = 480 corpus generations + 40 adversarial. 3-judge LLM ensemble scored blind. Gold anti-pattern citation rate on subtle tasks: T=72%, P=0%, B=0%. Overall verdict by pre-registered rules: inconclusive (no-memory baseline scored higher on holistic quality — confident guessing reads better than correct-but-messy retrieval to automated scorers). The measurement infrastructure works; the signal is real but not clean enough for the strict decision rules. Eval artifacts graph-indexed via `docs/eval-navigator.md`. | Initiative-wide: the graph provably helps for recall-dependent tasks. The eval's "inconclusive" verdict is honest and lands in the Élan case study Section 4 as the narrative closer. No plan changes needed — this is the final deliverable of the initiative's measurement track. |

## What this initiative does NOT include

- **Full A/B evaluation experiment** — COMPLETED. Ran as a self-contained automated effort. 4 arms, 12 tasks, 10 runs each, 3-judge LLM ensemble scoring blind. Verdict by pre-registered rules: inconclusive overall, but the graph found the gold anti-pattern 72% of the time on subtle tasks vs 0% for all other arms. Full report: [`docs/eval-results.md`](../eval-results.md). Artifact index: [`docs/eval-navigator.md`](../eval-navigator.md).
