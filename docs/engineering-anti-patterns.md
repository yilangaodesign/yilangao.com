...
| Build / Toolchain / CSS | EAP-011, EAP-012, EAP-031, EAP-035, EAP-038‡, EAP-039, EAP-040, EAP-069, EAP-072, EAP-077, EAP-078, EAP-080, EAP-088, EAP-112, EAP-116, EAP-123, EAP-127 | 17 active | 17 |
| Verification Discipline | EAP-113, EAP-115, EAP-122, EAP-126 | 4 active | 4 |
...
<a id="eap-126"></a>
### EAP-126: Declaring a visual component "done" on render-health checks without a visual/token-parity assertion

**Status: ACTIVE**

**Pattern:** A new visual component (or a visual change) is verified with the Mid-Flight gate's render-health checks — HTTP 200, clean hydration, no console errors, an interaction smoke test (e.g. sorting works) — and declared done. None of those checks compares the *rendered appearance* (typography scale, padding rhythm, header-vs-body hierarchy, token usage) against the system's existing equivalent component or the token spec.

**Why it fails:** "Renders correctly" and "looks right" are different assertions. Render-health checks are green whenever the component *runs*, but a component can run flawlessly while looking off — inverted type hierarchy (header smaller than body), cramped spacing, or hardcoded literals that bypass the token scale. Crucially, since every `var()` reference can resolve fine, a grep-for-token or console pass is *structurally incapable* of catching a value that's wrong-but-valid. The defect lives in the *literal values* and in *divergence from the canonical sibling*, which only a computed-style comparison or a side-by-side visual diff surfaces. ENG-269: DataGrid passed ENG-268's hydration/console/sort check and was declared done, then the user immediately reported it "looks very off" (10px headers smaller than 12px body, `padding: 0` header cells, cramped 4px density, a literal px ladder).

**Correct alternative:** For any visual DS component, extend the Mid-Flight gate with a visual/token-parity step *before* declaring done: (1) if a closely-related component already exists (here `Table`), open its `.module.scss` and compare the new component's header/cell typography + padding treatment against it; (2) inspect computed styles in-browser (font-size, padding, min-height, line-height on header and body cells) and confirm they match the intended scale — header font ≥ body font, padding on the token grid; (3) take a screenshot and eyeball hierarchy + rhythm. This is the visual analogue of EAP-113 (behavioral check, not grep) and EAP-115 (HTTP-200 doesn't exercise the broken thing).

**Detection:** A "done" report for a visual component whose verification section lists only HTTP status, hydration, console, and an interaction test — with no computed-style comparison, no sibling-component diff, and no screenshot review.

**Incident:** ENG-269 (2026-06-21, DataGrid "looks off" immediately after shipping). Cross-category with FB-248 / AP-074. Verification Discipline category 3 → 4.

---

### EAP-127: Adding a component to the DS package that imports an un-externalized dependency

**Category:** Build / Toolchain / CSS

**Pattern:** A new `src/components/ui/` component uses a package (e.g., `lucide-react` deep ESM imports) that resolves fine in the main site and playground (where the package is installed) but is not listed in `ds-package/tsup.config.ts` `external` or `ds-package/package.json` `dependencies`/`peerDependencies`. The local build gate tests the apps, not the ds-package build, so the gap is invisible until the GitHub Actions publish workflow fails.

**Why it fails:** The ds-package build runs `tsup` in an isolated `ds-package/node_modules` context. esbuild tries to resolve every import. If the package isn't in `external` (for consumer-provided deps) or `dependencies` (for bundled deps), the build fails with `Could not resolve "..."`. Local builds succeed because the apps' own `node_modules` trees have the package installed.

**Correct alternative:** When adding a new component to `ds-package/src/index.ts` that imports from a package not already in the ds-package dependency tree: (1) decide bundled vs external — icon libraries and large optional deps should be `external` + `peerDependencies` (optional); Radix primitives and small runtime deps should be `dependencies`; (2) update `tsup.config.ts` `external` array (use a regex like `/^lucide-react/` for packages with deep import paths); (3) run `cd ds-package && npm run build` locally to verify before pushing.

**Detection:** A `ds-package/src/index.ts` edit that adds exports for a component importing a new third-party package, without a corresponding `tsup.config.ts` or `package.json` change.

**Incident:** ENG-270 (2026-06-22, ds-package publish failed on lucide-react deep ESM imports). Build / Toolchain / CSS category 15 → 16.
