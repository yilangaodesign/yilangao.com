...
> **Last updated:** 2026-04-20 (ENG-153: Added `videoEmbed` block for external YouTube/Vimeo/Loom embeds via a new content block (Path A). Introduced `src/lib/parse-video-embed.ts`, `src/components/ui/VideoEmbed/`, and `VideoEmbedInput` inline editor. Path B (extending `Media` with `externalUrl`) rejected to preserve the Media collection's uploaded-file contract. Accepted gap: embeds cannot grid-mix inside `imageGroup` layout cells. See `docs/engineering/media-embeds.md`.)
...
| §12 | Media Storage & Preloading | `docs/engineering/storage.md` | Uploading files, storage, asset preloading pipeline |
...
| Build / bundler issues (React 19 compat) | 7 | **Critical — 3 failed fixes (ENG-017→018→019). Rule: no `<script>` in React tree. See EAP-013. ENG-117: Turbopack 16.2.x missing routes-manifest.json — switched main site to `--webpack` until upstream fix. See EAP-069. ENG-126 / ENG-127: SVG `fill="currentColor"` stale after client-side navigation. ENG-134: Corrupted `node_modules` caused webpack to silently fail generating ALL build artifacts (manifests, runtime). Full `rm -rf node_modules && npm install` required — clearing `.next` alone was insufficient. See EAP-072.** |
