...
> **Last updated:** 2026-06-21 (ENG-269: DataGrid "looks off" right after ENG-268 shipped — a verification-completeness gap, NOT a runtime bug. The Mid-Flight browser check passed render-health (HTTP 200, clean hydration, console, working sort) but never compared rendered typography/spacing against the sibling `Table` or the token scale, so wrong-but-valid literals + sibling divergence (10px headers, `padding: 0` header cells, cramped density, hardcoded borders/radius/line-height/px ladder) shipped. Fixed by mirroring `Table` + tokenizing every value; re-verified via in-browser computed-style inspection. Promoted EAP-126 + bumped the "Verification gap" frequency row 4 → 5 (new failure shape: render-health ≠ render-quality). Cross-category with FB-248 / AP-074.) Prior: 2026-06-21 (ENG-268: DataGrid v1 — headless TanStack Table v8 in a bespoke div-based WAI-ARIA grid (ADR-001 engine choice, ADR-002 substrate choice, both migrated to new spoke `docs/design/data-grid.md`). Second occurrence of "DS component scaffolding" frequency row (now 2) — extended for third-party-engine components: dep declared in every non-hoisting manifest + lucide deep-import type shim `src/types/lucide-icons.d.ts`. New `docs/backlog.md` deferred-task ledger added to the Section Index. Cross-category with FB-245.) Prior: 2026-06-21 (ENG-267: ButtonGroup DS component scaffolded through the full cross-app parity chain — both export barrels, `archive/registry.json`, playground page + sidebar, flush-restart-verified with clean hydration. New frequency row: "DS component scaffolding (dual-export + registry + playground parity)".) Prior: 2026-04-28 (ENG-266b: Self-Automated KG A/B Evaluation pipeline complete. 3 new frequency map rows: SHA-pinning timestamps, AI SDK tool schema API, LLM-judge fluency bias. New anti-pattern EAP-125.) Prior: 2026-04-27 (ENG-266: Knowledge-Graph-Initiative Plan C Phase 10 baseline-eval workflow pivoted from operator-mediated `cursor-export-transcript` (export → save → paste per run) to `cursor-jsonl` (`scripts/capture-eval-run.mjs` reads `~/.cursor/projects/<encoded>/agent-transcripts/<uuid>/<uuid>.jsonl` directly). Discovery prompted by user question with 8 of 9 runs still pending. JSONL strictly richer than export — preserves every tool call's full args. Frequency map: new row "Operator-mediated workflow designed without probing the data layer". Candidate AP added; promote on second occurrence.) Prior: 2026-04-24 (ENG-209 / EAP-120: Mixpanel crashed when `ProjectClient` tracked in `useEffect` before `AnalyticsProvider` finished `init` in its own `useEffect` (child passive effects run before parent). Appendix frequency map: new row "Client analytics SDK init ordering".)
...
| — | DataGrid Decisions | [`docs/design/data-grid.md`](design/data-grid.md) | DataGrid engine (ADR-001: TanStack vs AG Grid) & substrate (ADR-002: div-based ARIA grid) choices, TanStack wiring, lucide import rule |
| — | Backlog (Deferred Tasks) | [`docs/backlog.md`](backlog.md) | Looking for / recording deferred cross-cutting work not yet in a plan |
...
| DS component scaffolding (dual-export + registry + playground parity) | 2 | High — ENG-267, ENG-268: a new `src/components/ui/` component is done only after both export barrels (`src/components/ui/index.ts` for `@/` consumers AND `ds-package/src/index.ts` for `@ds/` consumers), `archive/registry.json`, and the playground page + sidebar entry are complete and flush-restart-verified. An export present in one barrel but not the other compiles locally yet breaks the playground import. **ENG-268 extends the chain for components that add a third-party engine:** the dependency must be declared in EVERY non-hoisting manifest (`ds-package/package.json` AND `playground/package.json` — the playground does not hoist from root), and any deep-import strategy (e.g. EAP-056 individual lucide imports) may need a type shim (`src/types/*.d.ts`) since `lucide-react` ships no `exports` map. A dep at root only, or an untyped deep import, compiles locally and fails only in the consuming app. ENG-268 also establishes that plan-embedded ADRs must be migrated to a graph-registered durable spoke (`docs/design/data-grid.md`) and deferred scope parked in `docs/backlog.md` before the plan is discarded. |
...
| Verification gap (reporting done without browser check) | 5 | **Critical — promoted to Hard Guardrail #10 (ENG-020). curl ≠ verification. ENG-189: 4th occurrence. ENG-188 closed with "browser verification deferred to next live session" — the next live session was the user re-reporting the identical bug 10 minutes later, because the 12h23m-old Webpack dev server had never delivered the structural React hook-shape changes. "Deferred verification" is a documented shortcut around Hard Guardrail #10, not a mitigation. EAP-115 now bans the phrase "deferred to next live session" as a closeout and mandates clean dev-server restart + compiled-chunk grep for any client-component hook-shape change. ENG-269 (5th): a NEW failure shape — verification DID happen in-browser (HTTP 200 + clean hydration + console + working sort all passed) but checked only render-*health*, never render-*quality*. DataGrid shipped looking "very off" (10px headers < 12px body, cramped density, hardcoded literals) because no step compared its computed typography/padding against the sibling `Table`. A browser check that doesn't assert visual/token parity is incomplete for a visual component. EAP-126.** |
...
| SHA-pinning files with embedded timestamps | 1 | **High — ENG-266. `build-graph.mjs` writes `generatedAt` into graph.json, making SHA different on every run. Eval runner's integrity check failed repeatedly. Fix: `EVAL_FREEZE_CACHE` env var to skip auto-rebuild. See EAP-125.** |
| AI SDK tool schema API mismatch | 1 | **Medium — ENG-266. AI SDK v6 uses `inputSchema` not `parameters` for `tool()`. Using `parameters` silently produces empty JSON Schema. Always test tool schemas with a minimal call before running at scale.** |
| LLM-judge fluency-over-factuality bias | 1 | **Medium — ENG-266. LLM judges gave higher adherence/quality scores to the no-memory baseline (Arm B) despite 6.0% gold-cite rate. Judges reward confident prose over domain-specific accuracy. Design decision rules around the metric with better reliability (preference kappa 0.53 vs Likert kappa 0.24).** |
| Operator-mediated workflow designed without probing the data layer | 1 | **High — ENG-266. Plan C Phase 10 locked in `cursor-export-transcript` as the eval evidence format — operator clicks Cursor's Export Transcript, saves to `~/Downloads/`, copy-pastes per run × 9 runs. The user pushed back mid-Phase-10 with 8 runs still pending; the JSONL was already at `~/.cursor/projects/<encoded-abs-path>/agent-transcripts/<uuid>/<uuid>.jsonl` AND was strictly richer than the export (preserves every tool call's full args, which Export Transcript drops as collapsed UI cards). Built `scripts/capture-eval-run.mjs` that derives the encoded path from the worktree absolute path, reads the most-recently-modified JSONL, validates the prompt against `docs/eval-task-corpus.md`, renders structured markdown with inlined tool calls, updates the manifest, and `git restore . && git clean -fd`-cleans the worktree. Operator workflow collapsed from "export → save → paste → clean" per run to "type 'T0XX-run-N done' in chat." Sixty-second prevention rule: when a workflow needs operator hands to bridge a data gap, run a filesystem audit (`ls ~/.<tool>/`, `ls ~/.cursor/`, `find ~/Library -name "*<tool>*"`) BEFORE locking in the design. The user-visible UI export is almost never the only path to the data; tools that ship a UI export almost always persist a richer underlying form on disk. Candidate AP added (promote on second occurrence). Strengthens EAP-122 (the inverse failure: data layer with no consumer). Cross-category note: engineering-only — pure tooling/methodology fix.** |
