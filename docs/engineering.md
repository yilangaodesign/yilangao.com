...
| Client-side storage for cross-session identity | 1 | **High — ENG-202 / EAP-119. `localStorage`-only owner tagging fails in incognito (storage clears on close). `distinct_id` changes after `identify()`. Fix: four-layer detection (admin password cookie, tag-owner URL cookie, `$device_id` match, Payload admin disable). Use `get_property('$device_id')` not `get_distinct_id()`.** |
