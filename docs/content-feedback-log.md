...
## Session: 2026-04-28 — Homepage subline derived from CMS category

<a id="cfb-047"></a>
#### CFB-047: "Why are those two wording or texts not connected?"

**Intent:** The homepage case study blurb showed a hardcoded string ("Élan Design System · Self-Improving AI Collaboration") from `src/lib/home-case-subline.ts` while the detail page sidebar showed the CMS `category` field ("Context Engineering · Agent Memory Design"). Updating one didn't update the other.
**Root Cause:** The homepage subline was a separate hardcoded map, not derived from CMS data. Historical artifact from before the CMS `category` field was stable.
**Resolution:** Removed `home-case-subline.ts` entirely. Homepage `subline` now reads `doc.category` from CMS — the same field the detail page sidebar renders. Single source of truth: edit `category` in the CMS (or the seeding route), and both surfaces update. Deleted the dead file.
**Cross-category note:** Also documented as ENG-267 (engineering — data flow desync between hardcoded map and CMS field).

---

## Session: 2026-04-28 — Élan hero metric rewrite

<a id="cfb-046"></a>
#### CFB-046: "I don't think 5K knowledge graph edges is a good metric... we have a very good money shot number"

**Intent:** The hero metric is the single biggest number on the sidebar — it needs to communicate *impact*, not *scale*. "~5K edges" describes size (how big is the graph) not outcome (does the graph work). The 72% eval result directly proves the system's value: it found the right principle 72% of the time when baselines scored 0%.
**Root Cause:** The hero metric was set before the eval results existed. When the eval completed, the results went into Section 4 body copy and the EvalComparisonGrid component, but the hero metric was never updated to match.
**Resolution:** Changed hero metric from `~5K / knowledge graph edges` to `72% / correct retrieval on subtle tasks`. Tooltip rewritten with eval methodology. Now aligns with Meteor (95% noise reduction) and Lacework (58% usability improvement) — all three case studies lead with outcome. Portfolio coherence manifest updated: Elan metric type changed from "Knowledge-graph scale" to "Percentage improvement" (same category as Lacework but measuring fundamentally different phenomena).
**Pattern extracted → portfolio coherence: hero metrics should prove impact, not describe size.**

---

## Session: 2026-04-28 — Élan category rewrite

<a id="cfb-045"></a>
#### CFB-045: "The tag here needs to be improved. What is this case study really about?"

**Intent:** The sidebar category tag is the fastest signal an interviewer reads. "Design Systems · Self-Improving AI Collaboration" buries the lead — "Design Systems" is generic (every UX designer has one) and "Self-Improving AI Collaboration" is abstract buzzword territory. Neither communicates what makes the case study unique.
**Root Cause:** Category was written when the case study was a design system showcase. After the knowledge graph, eval framework, and Section 4 rewrite, the core story shifted to context engineering and agent memory — but the category never followed.
**Resolution:** Changed to `Context Engineering · Agent Memory Design`. "Context Engineering" names the discipline (building the right context for AI, not just prompting it). "Agent Memory Design" names the specific problem the blurb and scope statement are about. Neither term appears in any other case study category — portfolio diversity maintained. Seeded to CMS and verified via Payload REST API.
**Pattern extracted → `content.md` §3: Category tags should name the insight, not the vehicle.**

---

## Session: 2026-04-28 — Narrative twist injection: 8-act draft reversals into case study sections

<a id="cfb-044"></a>
#### CFB-044: Élan narrative rewrite — bucket problem, rot, capture twist, section retitle

**Intent:** The user identified the built case study content as narratively flat. The 8-act draft had four twists the published copy completely dropped: the bucket problem (cross-pillar doubled overhead), rot as a named failure mode, the "felt good for two weeks" deflation, and the capture protocol near-miss (Act VII). The user wanted the twists injected into the existing 4-section structure without changing visual assignments.

**Root Cause:** The previous v4 copy (CF-032) prioritized clean structure over narrative tension. Each section summarized its acts without developing the reversals. Section 2 mentioned "silent decay" in one clause but didn't develop it. Section 3 jumped from graph description to "raises a question" with no emotional beat. Section 4 described the eval without the capture protocol twist.

**Resolution:**
- Section 1: Rewritten with vibe-coding setup ("cursor in one hand, deadline in the other"), specific failure examples (hardcoded hex, playground flush, em dashes).
- Section 2: Bucket problem developed (cross-pillar feedback, doubled documentation overhead). Rot named explicitly as a failure mode. SkillMap visual reframed as "the organized attempt that broke."
- Section 3: Retitled from "The Living Graph" to "Relationships Hidden in Context." Deflation beat added ("I felt good about this for about two weeks. Then I started asking the question I'd been avoiding."). GraphCanvas reframed as the iteration that replaced the flat structure.
- Section 4: Capture protocol twist added (Export Transcript vs JSONL on disk, meta-system builder's own workflow had the exact bug the meta-system was designed to prevent).
- INTERACTIVE_VISUALS key in page.tsx updated to match new Section 3 heading.
- Visual assignments, blurb, scope statement, hero metric unchanged.

**Anti-pattern relevance:** CAP-023 (Voice Flattening) — previous copy sanded down the draft's energy. CAP-027 (Fragment Chain) — Section 4 opening fragments woven into narrative sentences. CAP-028 (Technique-as-Posturing) — all sections use "I" as a person making choices, not architectural abstraction.

---

## Session: 2026-04-28 — Rising Floor visualization pivot: severity → recurrence, honest data framing

<a id="cfb-043"></a>
#### CFB-043: Rising Floor chart redesign — severity view removed, recurrence as default, hero metric revised

**Intent:** The Rising Floor section's MaturityTimeline chart was showing correction severity (fundamental/structural/refinement) as the primary view, but the severity classification tracked anti-pattern promotion status, not correction quality. The user wanted a visualization that directly proved the "agent memory" thesis — that the knowledge graph's coverage grows over time.

**Root Cause:** Three compounding issues: (1) Severity labels (fundamental/structural/refinement) sounded like they tracked correction difficulty but actually tracked whether the correction led to a new anti-pattern, a rule promotion, or neither. The visual metaphor was misleading. (2) The "15 → 3" hero metric was manually derived early in the project and couldn't be reproduced from the API data — total corrections per day actually trend upward as project scope expands. (3) The recurrence classification (recurrent/approximate/novel) directly measures knowledge coverage but was buried as the third toggle option.

**Resolution:**
- Removed severity view entirely from MaturityTimeline. Recurrence is now the default view (percentage Y-axis, stacked bars showing recurrent/approximate/novel as % of daily total, novel-% trend line). Domain remains as secondary toggle (absolute counts).
- Fixed `parseContentLog` to classify recurrence per individual CFB entry rather than per session block (was inflating recurrent counts).
- Hero metric changed from "15 → 3" (Avg corrections / session) to "130+" (anti-patterns encoded). The new metric is verifiable and honest.
- Scope statement rewritten to remove the unsupported declining-count claim. Now frames the chart as "every correction either cites an existing rule or surfaces a gap."
- Rising Floor body copy rewritten: shifts from specific correction examples to the coverage observation.
- Data verification confirmed novel-% does NOT decline — it trends slightly upward (+0.34%/day) because the project keeps entering new territory. Copy was written to match this honest reality rather than claiming a trend that doesn't exist.
- Updated downstream: dossier (elan-design-system.md), portfolio-coherence.md, interview prep (elan-design-system-interview.md).

**Anti-pattern relevance:** CAP-025 (Floating Metric) — the old "15 → 3" had no live derivation path. The new "130+" is anchored to the knowledge graph's actual count documented in the scope statement.
...
#### CF-032: "Case study visual strategy redesign — 4-section restructure"

**Intent:** Restructure the Élan case study from a 4-section layout (3 visuals + 1 temporal proof) to a 4-section layout (3 visuals + 1 text-only closer) with a redistributed narrative arc: mechanism → scale + decay → the graph → the question.

**Root Cause:** The original Section 4 ("The Rising Floor" / MaturityTimeline) showed correction recurrence data that didn't trend meaningfully — the novelty rate didn't decline as expected. The chart was the weakest evidence and contradicted the narrative's claim of compounding knowledge. Additionally, the experiment and measurement story (40% of the narrative draft's substance) was compressed into Section 3's tail.

**Resolution:**
- Removed MaturityTimeline from INTERACTIVE_VISUALS and VISUAL_COMPONENTS (dead code stays in repo)
- Renamed Section 1 from "Not the Components" to "Teach Once, Enforce Forever" (human agency framing)
- Rewrote all section bodies to channel the 8-act narrative draft
- Added Section 4 "Is Any of This Working?" as text-only closer (3 paragraphs: the question, the experiment, the thesis)
- Updated hero metric from "130+" (anti-patterns) to "~5K" (knowledge graph edges)
- Updated all metrics: 16→18 skills, 130+→200+ anti-patterns, added ~4,892 edges
- Added GraphCanvas auto-transition: Signal→Pathway→Mesh loop with hub rotation, viewport-gated, node-click cancellation
- Added signalSourceOverride prop to ForceGraph for external hub control
- Made SkillMap Operations heading dynamic (reads skill count from API)

**Pattern extracted:** When a data visualization contradicts the narrative, removing it and replacing with a stronger structural element (text-only honest question) is better than forcing the data to tell a story it can't support. The deliberate absence of a visual after three interactive sections creates a gear shift that functions as punctuation.
