...
- `recent-feedback.canvas` — last 30 entries per active feedback log + cited APs (~143 nodes), edges colored by confidence (strong / loose). Visual mirror of the `MaturityTimeline` "By recurrence" classification.
...
**When to add a richer UI.** When the canvases stop scaling (filtering needs, deep-linking from outside Obsidian, public-facing views), the next step is an in-site `KnowledgeGraph.tsx` viewer mirroring the `MaturityTimeline` pattern — Cytoscape/Sigma rendering of the same `.cache/graph.json` consumed by `/api/knowledge-graph/route.ts`. That work is intentionally deferred until the canvases prove insufficient.
