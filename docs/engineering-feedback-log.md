...
<a id="eng-270"></a>
### ENG-270: DS package publish workflow failed — lucide-react deep ESM imports not externalized in tsup config

**Date:** 2026-06-21

**Issue:** GitHub Actions `Publish Design System` workflow failed on push to main after REL-040. The tsup build in `ds-package` errored with 8 `Could not resolve "lucide-react/dist/esm/icons/*"` errors from DataGrid sub-components (`DataGridPagination.tsx`, `DataGridFilterRow.tsx`, `DataGridColumnHeader.tsx`).

**Root cause:** DataGrid components use EAP-056-compliant deep ESM imports (`import ChevronLeft from "lucide-react/dist/esm/icons/chevron-left"`) to avoid Turbopack barrel-export hydration mismatches. The `ds-package/tsup.config.ts` external array only listed `react`, `react-dom`, `framer-motion`, and `next` variants — `lucide-react` was missing. The local build gate (playground, main site, ASCII tool, Edra) all passed because they have `lucide-react` installed; the ds-package build runs in an isolated `ds-package/node_modules` context where it was never added.

**Resolution:** Added `/^lucide-react/` regex to the `external` array in `ds-package/tsup.config.ts` (regex catches all deep paths). Added `lucide-react` as an optional `peerDependency` in `ds-package/package.json` (optional because only DataGrid consumers need it). Verified local ds-package build succeeds. Pushed fix to main to re-trigger the publish workflow.

**Principle:** When adding a new component to the DS package that imports from a package not already in the ds-package dependency tree, add it to either `dependencies` (if the package should be bundled) or `external` + `peerDependencies` (if consumers provide it). The local build gate does not catch ds-package build failures because it tests the apps, not the package.

**Files modified:** `ds-package/tsup.config.ts`, `ds-package/package.json`

---

<a id="eng-269"></a>
### ENG-269: DataGrid "looks off" right after shipping — verification passed HTTP-200 + clean hydration but never checked visual/token parity against the sibling component

**Date:** 2026-06-21

**What happened:** Immediately after ENG-268 shipped DataGrid v1 (with a flush-restart + in-browser hydration/console check that all passed), the user reported it "looks very off" and "hasn't used all the tokens." The audit (FB-248) confirmed: header font fixed at 10px (smaller than the 12px body — inverted hierarchy), `padding: 0` on header cells (non-sortable headers jammed to the edge), cramped 4px standard density, and hardcoded literals (`border: 1px`, `border-radius: 0`, `line-height: 1.4`, a `20/26/34/50px` density ladder) where tokens exist.

**Root cause:** A **verification-completeness gap**, not a runtime bug. ENG-268's Mid-Flight check confirmed the component *rendered without errors* (HTTP 200, no hydration mismatch, no console warnings, sorting worked) — but "renders correctly" and "looks right" are different assertions. Nothing in the check compared the rendered typography/spacing against the system's existing `Table` component or against the token scale. Every `var()` reference resolved fine, so a grep/console pass was always going to be green; the defect lived in the *literal values* and in *divergence from the canonical sibling*, which only a computed-style comparison or a side-by-side visual diff would catch. This is the visual-quality analogue of EAP-113 (verification-by-grep-without-behavioral-check) and EAP-115 (HTTP-200 doesn't exercise the thing that's broken).

**Resolution:**
- Rewrote `DataGrid.module.scss` to mirror the canonical `Table` treatment and tokenize every value: header font tracks `--dg-font` (12px at md), header cells inherit `.cell` padding (sort `<button>` zeroed so it doesn't double), density ladder snapped to `spacer-3x/4x/5x/6x` with `spacer-0-25x/0-5x/1x/1-5x` vertical padding, borders → `--portfolio-border-width-thin`, corners → `--portfolio-radius-none`, line-height → `--portfolio-leading-snug`, focus ring + status accent → `--portfolio-border-width-regular`.
- Flush-restarted the playground (EAP-042), confirmed compiled CSS carries the token refs, and re-inspected computed styles in-browser: header now 12px / `8px 16px` padding / semibold, 40px standard rows, comfortable rhythm, density ladder graduated correctly.
- Updated `docs/design/data-grid.md` density table + token-discipline note and the playground density caption.

**Engineering principle learned:** For a *visual* DS component, the Mid-Flight verification gate must include a visual/token-parity assertion, not just a render-health assertion. "HTTP 200 + clean hydration + no console errors" proves the component *runs*; it does NOT prove it *belongs*. When a closely-related component already exists (here `Table`), the cheapest correct check is a computed-style comparison of the new component's header/cell typography + padding against that sibling. Promoted EAP-126.

**Cross-category note:** Also documented as FB-248 (design — the inverted hierarchy / cramped rhythm / token-discipline failure) and AP-074 (the "hardcode + diverge from sibling" anti-pattern).

---

<a id="eng-268"></a>
### ENG-268: DataGrid component (v1) — TanStack engine, div-grid substrate, and the full parity + durable-docs chain

**Date:** 2026-06-21

**What happened:** Built the Élan `DataGrid` v1 (`src/components/ui/DataGrid/`) — a headless TanStack Table v8 model rendered into a bespoke div-based WAI-ARIA grid (sorting, per-column filtering, row selection, pagination, full keyboard nav). Wired it through every monorepo layer and preserved the architectural decisions in durable docs.

**Root cause:** N/A — proactive, plan-driven feature work, not an incident. Logged because it crosses the Cross-App Parity gate, adds a third-party dependency, and generates reusable engineering knowledge (engine choice, render substrate, dependency hoisting, icon typing).

**Resolution (engineering chain executed):**
- **Engine + substrate decisions:** TanStack Table v8 over AG Grid (ADR-001 — headless = total token control, MIT, no license, virtualization via `react-virtual`; AG Grid's differentiating features are paywalled Enterprise and impose its own DOM/theme). Div-based `role="grid"` over native `<table>` (ADR-002 — virtualization / sticky / resize are hostile to table layout; the trade is that WAI-ARIA keyboard support becomes a non-optional build item). Both ADRs migrated verbatim into the new durable spoke (see below).
- **Dependency hoisting:** Added `@tanstack/react-table` to BOTH `ds-package/package.json` AND `playground/package.json`. The playground does NOT hoist from the root `node_modules`, so a dep present only at root resolves locally but breaks the playground's `@ds/*` import at build — the same two-surface trap as ENG-267's export barrels.
- **Lucide icon typing:** The grid's sort caret + pager chevrons use individual deep imports (`lucide-react/dist/esm/icons/<name>`, per EAP-056 to avoid Turbopack SSR/client icon mismatches). `lucide-react` ships no `exports` map, so `moduleResolution: "bundler"` could not type the deep `.mjs` path. Added `src/types/lucide-icons.d.ts` (ambient `declare module "lucide-react/dist/esm/icons/*"`), mirroring the pre-existing shim in `playground/src/types/`.
- **Parity:** Exported from `src/components/ui/index.ts` (`@/` consumers) AND `ds-package/src/index.ts` (`@ds/` consumers); registered `shared-ui-data-grid` in `archive/registry.json`; playground page + sidebar entry (with `keywords` for fuzzy search) added.
- **Verification (Mid-Flight Check 1):** Killed playground, `rm -rf playground/.next`, restarted, polled HTTP 200, curl-grepped `/components/data-grid` for distinctive strings, and opened it in-browser — clean hydration, no console/icon errors, click-to-sort confirmed reordering rows. Port registry PID refreshed.
- **Durable docs:** Created `docs/design/data-grid.md` spoke (ADR-001, ADR-002, prop-naming, density→row-height mapping, cell conventions, v2 scope), cross-referenced from both the design and engineering Section Indexes. Established `docs/backlog.md` as the deferred-task ledger (seeded the keywords-backfill + DataGrid-v2 entries). `build-graph` clean (0 warnings); `audit-docs` introduced 0 new errors/warnings.

**Engineering principle learned:** For a component that adds a third-party engine, the parity chain is wider than ENG-267's: it is the two export barrels + registry + playground page + sidebar AND the dependency declared in every non-hoisting manifest AND any type shim the deep-import strategy requires. A dep added only at root, or a deep import left untyped, compiles locally and fails only in the consuming app. Separately: a plan is an ephemeral execution vehicle — architectural decisions made inside it (ADRs) MUST be migrated into a graph-registered durable spoke before the plan is discarded, or the "why" evaporates. The backlog ledger is the home for deferred scope so it survives plan deletion.

**Cross-category note:** Also documented as FB-245 (design — the DataGrid visual axes, `numeric`/`zebra` prop-naming decisions, and cell-content composition model).

---

<a id="eng-267"></a>
### ENG-267: ButtonGroup component scaffolding — exports, registry, and playground parity

**Date:** 2026-06-21

**What happened:** Created a new DS component `ButtonGroup` (`src/components/ui/ButtonGroup/`: `ButtonGroup.tsx`, `ButtonGroup.module.scss`, `index.ts`) and wired it through every layer of the monorepo so it is consumable, discoverable, and documented.

**Root cause:** N/A — proactive feature work, not an incident. Logged because component creation in `src/` triggers the Cross-App Parity gate and generates reusable engineering knowledge.

**Resolution (parity checklist executed):**
- **Component:** Presentational flex container — `forwardRef<HTMLDivElement>`, `HTMLAttributes` passthrough, `role="group"`, no `typeof window` / client branching (zero hydration surface). SCSS uses `@use '../../../styles' as *` and `var(--portfolio-spacer-*)` tokens for gaps; no inline styles, no border-collapsing.
- **Exports:** Added `ButtonGroup` + its types to `src/components/ui/index.ts` (local `@/components/ui` consumers) AND `ds-package/src/index.ts` (so the playground's `@ds/*` alias resolves).
- **Registry:** Added `shared-ui-button-group` to `archive/registry.json`.
- **Playground:** New `playground/src/app/components/button-group/page.tsx` thin harness (imports the real component via `@ds/*`, demos every prop via `ComponentPreview`, documents the API via `PropsTable`) + sidebar nav entry in `playground/src/components/sidebar.tsx` using an individually-imported Lucide icon (`lucide-react/dist/esm/icons/group`, per EAP-056 — no barrel import).
- **Verification (Mid-Flight Check 1):** Killed playground, `rm -rf playground/.next`, restarted, polled HTTP 200, curl-grepped the page for the component + props, confirmed the Button page still renders, and opened the page in-browser to confirm clean hydration. The Next.js Dev Tools "1 Issue" was a **pre-existing** sidebar-shell hydration warning (`src/components/sidebar.tsx:886 @ Sidebar`, a `typeof window` collapse-state branch) present site-wide and unrelated to this change — an additive nav-link entry cannot introduce a `typeof window` branch.

**Engineering principle learned:** A new `src/` component is not "done" at the `.tsx` file — it is done when it has cleared the full parity chain (both export barrels, registry, playground page + sidebar) and the playground has been flush-restart-verified. The two export surfaces (`src/components/ui/index.ts` for `@/` consumers and `ds-package/src/index.ts` for `@ds/` consumers) are easy to half-complete; an export present in one but not the other compiles locally yet breaks the playground import.

**Cross-category note:** Also documented as FB-246 (design — the spaced-vs-fused intent model and §1.3 enforcement rationale).

---

<a id="eng-266"></a>
### ENG-266: Baseline eval transcript capture — designed an Export-Transcript + copy-paste workflow without probing the JSONL on disk first

**Date:** 2026-04-27

**Issue:** Plan C Phase 10 of the Knowledge Graph Initiative locked in `cursor-export-transcript` as the canonical evidence format for the 9 baseline runs. The procedure: operator runs the prompt in the worktree window, clicks Cursor's "Export Transcript", saves the markdown to `~/Downloads/`, then copy-pastes it into `docs/eval-baselines/current/<task>/run-<n>.md`. Eight runs of the nine were still pending when the user pushed back: "what if every time I copy and paste something, you go into that file or folder and just document yourself? what if you can access the transcript with the granularity of what I am just doing with copy and pasting, but in a more structured way?"

The question forced two simultaneous discoveries:

1. **The JSONL is already on disk.** Each Cursor window persists its chat as `~/.cursor/projects/<encoded-abs-path>/agent-transcripts/<uuid>/<uuid>.jsonl`. The encoded-path scheme converts `/Users/yilangao/eval/yilangao-current` to `Users-yilangao-eval-yilangao-current` (slashes → hyphens, leading slash dropped). The system prompt for THIS chat literally names the analogous path for the parent project (`Users-yilangao-Desktop-Job-Application-Vibe-Coding-yilangao-com`). I never generalized from "MY transcripts live at this path" to "the worktree window's transcripts live at the analogous path."
2. **The JSONL is strictly richer than the export.** Cursor's Export Transcript renders tool calls as collapsible UI cards in the chat, then drops them on export — a 51-tool-call run exports as ~10 KB of narrative text with ✓ markers where calls were. The JSONL preserves every tool call's full arguments (`Read` paths, `Grep` patterns, `Shell` commands) — same run is ~48 KB of structured events. The eval's "tool calls to relevant context" metric, which the future LLM-as-judge will read, is *invisible* in the export and *fully present* in the JSONL.

**Root cause:** I designed a workflow around the user-visible export feature without auditing whether the underlying data was already accessible. The bias was: "Cursor exports are the canonical way to extract chat content, so the workflow goes through Export Transcript." That bias survived past the point where the system prompt itself named the data location for an adjacent context (the parent project's transcript path) — a one-step generalization away. Eight runs × ~5 minutes of operator copy-paste per run = 40 minutes of avoidable manual work, plus loss of the highest-signal data (tool args) on every captured run.

**Resolution:** Wrote `scripts/capture-eval-run.mjs` (Node ES module). It:

1. Computes the worktree's encoded project name from the worktree path (`/Users/yilangao/eval/yilangao-current` → `Users-yilangao-eval-yilangao-current`).
2. Reads the most-recently-modified JSONL under `~/.cursor/projects/<encoded>/agent-transcripts/`.
3. Extracts the first user message, strips Cursor's `<timestamp>...<user_query>` wrapper, and validates the prompt matches the corresponding `prompt:` field in `docs/eval-task-corpus.md` (case- and whitespace-normalized). On mismatch, refuses to write — operator either ran the wrong prompt or is trying to capture an older JSONL.
4. Renders every assistant turn as markdown, with each tool call inlined as `> **<ToolName>** <arg-summary>` (paths stripped to worktree-relative; `Shell` commands de-prefixed of the `cd <worktree> && ` boilerplate; `Read` ranges shown as `(offset: N, limit: M)`).
5. Updates `docs/eval-baselines/current/manifest.json` (`captured: true`, `captured_at`, `transcript_source_uuid`, plus the running `runs_captured: "N/9"` aggregate).
6. Runs `git restore . && git clean -fd` in the worktree, then verifies `git status --porcelain` is empty.

Switched the canonical format from `cursor-export-transcript` to `cursor-jsonl` everywhere: `manifest.transcript_format`, `manifest.transcript_format_note`, `docs/eval-baselines/README.md` "Agent invocation procedure" section, and the 8 staged run-file templates. Re-captured T002-run-1 in the new format. Captured the remaining 8 runs (T002-2/3, T008-1/2/3, T011-1/2/3) by operator notification only — the operator pasted the prompt in the worktree window, the run completed, the operator typed "T0XX-run-N done" in this chat, and `node scripts/capture-eval-run.mjs --task eval-T0XX --run N` did the rest.

**Verification gate:** `node scripts/verify-eval-handle.mjs` passes 11/11 checks (Plan A+B commits between `eval-baseline-current` tag and HEAD, pillar mix 4/4/4, difficulty mix 6/6, prompt-leakage clean, manifest schema valid, all 9 run files present with populated frontmatter, README authored with Phase 00 SHA, all three node types declared in `docs/knowledge-graph.md`). Runs span 43-85 tool calls per file (median ~50); citation accuracy across all 9 runs: gold AP cited in 9/9, foil cited in 0/9, T011 trilogy specifically cites CAP-029 in all three runs and never cites CAP-017 (the foil). Worktree pristine after every capture (independently verified by direct `git status` against the `eval-baseline-current` tag SHA — 0 uncommitted, 0 diff lines).

**Files added:**
- `scripts/capture-eval-run.mjs` — capture + manifest-update + worktree-clean
- 8 newly populated `docs/eval-baselines/current/eval-T{002,008,011}/run-{1,2,3}.md` files

**Files modified:**
- `docs/eval-baselines/current/manifest.json` — `transcript_format: cursor-jsonl`, all 9 runs marked captured, `runs_captured: "9/9"`, `captured_window_start/_end` set, `captured_by: yilangao`
- `docs/eval-baselines/README.md` — "Agent invocation procedure" rewritten around the script; "Transcript-format limitations" updated to describe `cursor-jsonl`; "Common partial states" table aligned with the automated workflow
- `docs/eval-baselines/current/eval-T002/run-1.md` — re-captured under the new format

**Mid-Flight verification step that was missing:** When designing any workflow that requires the operator to perform repeated manual actions to bridge a data gap (export → save → paste, screenshot → upload, manually-collated logs), pause and run a 60-second filesystem audit before locking in the design — `ls ~/.cursor/`, `ls ~/.<tool>/`, `ls /tmp/<tool>*`, `find ~/Library -name "*<tool>*"`. The user-visible UI feature is almost never the only path to the underlying data; tools that ship a UI export almost always persist a richer underlying form on disk. The instinct should be: "where does this tool put its data when I'm not looking?" before "what does this tool let me click to extract its data?"

A complementary heuristic: when the system prompt or context names a data path for context A, ask "what's the analogous path for context B?" before designing around context B's UI. The encoding scheme is usually deterministic and learnable from one example.

**Principle:** Operator-mediated workflows are a last resort, not a default. Before locking in a design that requires the operator to copy-paste, click-export, or otherwise bridge data with their hands, audit whether the data is already accessible programmatically. The audit should happen at workflow-design time, not after the operator has done the work eight times. UI exports are user-facing summaries — they discard fidelity that the underlying persistence layer preserves. The right design hierarchy is: (1) read directly from disk if a path exists or can be derived; (2) call the tool's API/CLI if one exists; (3) only fall back to operator-mediated UI workflows when no programmatic access exists, and document the friction explicitly.

**Anti-pattern:** Candidate added to [`docs/candidate-anti-patterns.md`](candidate-anti-patterns.md) — "Operator-mediated workflow designed without probing the underlying data layer first." Promote on second occurrence.

**Strengthens:** Related: EAP-122 (data-layer-without-consumer drift — the inverse failure mode: data exists, no UI consumer rendered it; here UI export was treated as the consumer when richer data existed).

Cross-category note: engineering-only — pure tooling/methodology fix. The format pivot doesn't affect the eval's pillar mix (4 design / 4 engineering / 4 content) or any case-study content.
...
**Issue:** Console error: `flushSync was called from inside a lifecycle method. React cannot flush when React is already rendering.` at `SkillMap.tsx:113`. Fires on every mount of the SkillMap component.

**Root cause:** `OperationsGrid` uses `flushSync(() => setExpanded(name))` inside a `useLayoutEffect` to synchronously expand each skill card, measure the grid height, and lock in the tallest value before paint. React 19 forbids `flushSync` during the commit phase (layout effects run during commit). The pattern worked in React 18 but is now a warning/error in 19.

**Resolution:** Wrapped the `flushSync` measurement loop in `queueMicrotask()` inside the `useLayoutEffect`. Microtasks execute after the current task (React's commit phase) completes but before the browser paints — so `flushSync` no longer runs during a lifecycle method (eliminating the error), while intermediate expansion states remain invisible to the user. No change to the measurement logic itself: each card is still expanded synchronously, measured, and collapsed, with `gridFixedHeight` locked to the tallest value.

**Files modified:** `src/components/elan-visuals/SkillMap.tsx`

**Principle:** When React 19 forbids `flushSync` inside lifecycle methods, `queueMicrotask` is the correct escape hatch for pre-paint synchronous measurement — it exits the commit phase but fires before the browser's next paint, preserving the "invisible intermediate states" guarantee that `useLayoutEffect` + `flushSync` originally provided.
...
**Issue:** MaturityTimeline displayed two separate charts (grouped-period stacked bars + trend band). User wanted a single combined chart with per-build-day granularity showing both composition and trend simultaneously.

**Root cause:** The API only returned aggregate per-period data. To show one bar per build day, the API needed per-day severity + domain breakdown. The previous two-chart approach also required viewers to context-switch between volume and trend.

**Resolution:**
- Extended `/api/maturity` with a new `DayBreakdown` type and `days` array: each entry has `day` (1-indexed), `date`, severity counts (fundamental/structural/refinement), domain counts (design/engineering/content), and `total`
- Built `dayBreakdownMap` by iterating `allCorrections`, then mapped `uniqueDates` to produce the sorted `days` array
- Rewrote `MaturityTimeline.tsx`: replaced grouped-period bars with one bar per build day; SVG polyline overlay for rolling-median trend (25% window); grayscale DS tokens for segments; hover tooltip with day-level breakdown
- Rewrote `maturity-timeline.module.scss`: new `.combinedChart`, `.barsLayer`, `.dayColumn`, `.dayBar`, `.daySegment`, `.grayDark/Mid/Light`, `.trendOverlay`, `.trendLine`, `.tooltip` etc.

**Files modified:** `src/app/(frontend)/api/maturity/route.ts`, `src/components/elan-visuals/MaturityTimeline.tsx`, `src/components/elan-visuals/maturity-timeline.module.scss`

Cross-category note: Also documented as FB-207 (design).
...
**Issue:** MaturityTimeline rendered static invented numbers (e.g. "Sessions 1-5: fundamental: 12") that didn't reflect actual correction history.

**Root cause:** The component was authored with placeholder data before the feedback-log parsing infrastructure existed. The same issue was previously solved for the AntiPatterns treemap (ENG-213) and SkillMap (live skill reads), but MaturityTimeline was left behind.

**Resolution:** Created `src/app/(frontend)/api/maturity/route.ts` — a Next.js Route Handler that:
- Parses `**Date:** YYYY-MM-DD` entries from `design-feedback-log.md`, `engineering-feedback-log.md`, and `engineering-feedback-log-archive.md` (split by `---`, one entry per block)
- Parses `## Session: YYYY-MM-DD` + `#### CFB-NNN:` sub-entries from `content-feedback-log.md`
- Derives unique session dates chronologically, assigns sequential session numbers, and groups into periods (1–5, 6–10, 11–15, 16–20, 21–30, 31–40+)
- Classifies each entry as fundamental (promoted a new anti-pattern), structural (cross-category or long), or refinement (everything else)
- Auto-detects "First zero-fundamental session" milestone; "Escalation protocol introduced" is a static editorial annotation on the 6–10 period
- Returns `s-maxage=60, stale-while-revalidate=300` JSON; verified live: 21 sessions, 5 periods with data

Updated `MaturityTimeline.tsx`: removed all static data constants; added `useEffect` fetch of `/api/maturity` on mount; loading skeleton shows while fetch is in-flight; chart renders from live data. The `period` field now maps to the short label ("1-5") directly from the API, removing the `.replace("Sessions ", "")` workaround.

**Files modified:** `src/app/(frontend)/api/maturity/route.ts` (new), `src/components/elan-visuals/MaturityTimeline.tsx`, `maturity-timeline.module.scss` (`.chartLoading`)
...
**Issue:** Hovering the bottom bar segments (e.g. "Fundamental" in severity mode) showed no tooltip — the popover appeared to render behind the chart.

**Root cause:** CSS stacking context conflict. `flex-direction: column-reverse` reverses visual order but not DOM order. In the DOM, `fundamental` comes first, then `structural`, then `refinement`. All three `.barSegment` elements have `z-index: 1`, establishing individual stacking contexts. Since `structural` and `refinement` appear later in the DOM, they are painted on top of `fundamental` at the same z-level, covering its popover entirely. The popover's own `z-index: 20` is scoped inside `fundamental`'s stacking context and cannot escape it.

**Resolution:** Added `.barSegmentHovered { z-index: 10 }`. When the hovered segment (regardless of DOM position) receives this class, its stacking context at z=10 beats all sibling segments at z=1. The popover (z=20 within the elevated context) is now always visible above sibling segments. Class applied conditionally via `isHovered` flag in `MaturityTimeline.tsx`.
...
**Issue:** User requested the treemap reflect real-time anti-pattern counts from the docs rather than hardcoded data. The hardcoded data was also stale (136 grand total vs actual 178).

**Root Cause:** `TREEMAP_DATA` and `FLAT_CATEGORIES` were static constants in `SkillMap.tsx`; the Category Index tables in the three anti-pattern docs are updated by every documented fix but were never fed back into the visualization.

**Resolution:** Created `src/app/(frontend)/api/antipatterns/route.ts` — a Next.js Route Handler that `readFileSync`s `docs/design-anti-patterns.md`, `docs/engineering-anti-patterns.md`, and `docs/content-anti-patterns.md`, parses each `## Category Index` markdown table (regex on last numeric cell per row, skips header/separator/Total rows), and returns structured JSON with per-category counts, per-domain totals, and a grand total. Response uses `s-maxage=60, stale-while-revalidate=300`. Updated `SkillMap.tsx` to `useEffect`-fetch `/api/antipatterns` on mount; `KnowledgeTreemap` now accepts `{ categories, totals }` props (no more global constants); `SkillMap` header shows the live grand total; treemap layout `useMemo` deps include `categories` so it recomputes on data arrival. Fallback: blank skeleton while loading, no render on fetch error. `AntipatternData` type exported from the route for use in the client.

**Files modified:** `src/app/(frontend)/api/antipatterns/route.ts` (new), `src/components/elan-visuals/SkillMap.tsx`, `skill-map.module.scss` (`.treemapLoading`)
...
**Issue:** Hovering cells on the right side of the Knowledge treemap (e.g. "Dev Workflow") showed a tooltip that was cut off by the treemap frame; meta text read as `"Engineering / 5 an…"` with the right side of the chip invisible.

**Root Cause:** `.treemapContainer` used `overflow: hidden`, which clipped absolutely positioned descendants. The tooltip is centered on the cursor with `transform: translate(-50%, -100%)`, so half of the chip naturally extends past the right boundary when the cursor sits on a right-edge tile.

**Resolution:** (1) Set `overflow: visible` on `.treemapContainer` and `.visualizationWithLegend`. (2) Raised `.treemapTooltip` `z-index` to 20. (3) Added `max-width: min(280px, calc(100% - 2x spacer))`, `white-space: normal`, and `word-wrap: break-word` so long lines wrap inside the chip. (4) Added `clampTreemapTooltipX()` in `SkillMap.tsx` so the anchor X stays within `[edge + halfEstimate, width - edge - halfEstimate]` on `mouseenter` / `mousemove` (half estimate 130px, matches wrapped max width order of magnitude).

**Files modified:** `src/components/elan-visuals/skill-map.module.scss`, `SkillMap.tsx`

**Cross-category note:** Same failure family as ENG-211 (overflow clipping portaled or absolute UI).
...
**Issue:** User reported the value tooltip no longer appeared on hover over the maturity chart.

**Root Cause:** ENG-210 moved the popover to `left: 100%` of the segment. Parent `.barStack` still had `overflow: hidden` (legacy from rounded bar tops), which **clipped** any popover painted outside the stack’s box, so the tooltip was fully invisible. After reverting popover placement to above the segment, moving the cursor toward the label still crossed a gap where `pointerLeave` cleared hover unless the hit target was extended.

**Resolution:** (1) Set `overflow: visible` on `.barStack`, `.barColumn`, and `.chartArea`. (2) Restored popover to `bottom: calc(100% + 0.5x)` / centered above the active segment with `z-index: 20` and `z-index: 1` on `.barSegment` so it stacks above the bridge. (3) Added an in-flow absolutely positioned `.barStackHoverBridge` (56px tall, `aria-hidden`) above the stack so `onPointerMove` on `.barStack` keeps resolving the top segment while the cursor moves from a slice toward the popover.

**Files modified:** `src/components/elan-visuals/maturity-timeline.module.scss`, `MaturityTimeline.tsx`

**Cross-category note:** Supplements ENG-210 / EAP-121; side-only anchoring is unsafe when any ancestor uses `overflow: hidden`.
...
**Issue:** Hovering a stacked bar segment showed the value popover only briefly; it vanished as the pointer moved slightly.

**Root Cause:** Each `.barSegment` used `onMouseEnter` / `onMouseLeave`. The popover was absolutely positioned above the segment (`bottom: calc(100% + 8px)`). Moving the cursor toward the label (or the tooltip) left the segment’s small hit target, firing `mouseLeave` and clearing hover. `pointer-events: none` on the popover did not extend the hit target.

**Resolution:** (1) Removed per-segment enter/leave handlers. (2) Added `segmentKeyAtStackPointer()` and wired `onPointerMove` / `onPointerLeave` on `.barStack` to map `clientY` to a segment using cumulative heights from the stack’s bounding rect (`yFromBottom`), with the last rendered slice absorbing floating-point drift so heights partition the full stack. (3) Repositioned `.popover` to the right of the active segment (`left: 100%`, `top: 50%`, `margin-left: $portfolio-spacer-1x`, `transform: translateY(-50%)`) so the cursor path no longer crosses dead space above thin slices.

**Files modified:** `src/components/elan-visuals/MaturityTimeline.tsx`, `maturity-timeline.module.scss`

**Cross-category note:** FB-182 (design) documents removal of portfolio-forbidden `border-radius` on the same chart (bar stack, milestone chip, legend dots, popover).
...
**Issue:** Runtime crash when opening a case study: `Cannot read properties of undefined (reading 'before_track')` at `mixpanel.track` from `ProjectClient` `useEffect` (`Case Study Viewed`).

**Root Cause:** `initMixpanel` lived only in `AnalyticsProvider`'s `useEffect`. React runs **child** `useEffect` callbacks before **parent** passive effects on mount, so `ProjectClient` called `track()` before `mixpanel.init()`, leaving Mixpanel's internal hook registry undefined.

**Resolution:** Call `initMixpanel({ disable: isAdmin })` synchronously during `AnalyticsProvider`'s client render (`typeof window !== "undefined"`) so init completes before any descendant's passive effects run. Removed the redundant init from that `useEffect` (identify-only effect retained). Added `if (!initialized || disabled) return` guards on `track` and `identifyCompany` so orphan callers no-op instead of throwing.

**Files modified:** `src/components/AnalyticsProvider.tsx`, `src/lib/analytics/mixpanel.ts`

**Principle:** Client analytics SDKs that require `init` before `track` must not rely on parent `useEffect` ordering alone when children also emit events from effects; use render-time init (client-guarded) or another ordering-safe bootstrap.
...
**Issue:** (1) The input field at `max-width: 360px` inside a `max-width: 380px` card was too narrow to display "glad you are here" at 2.75rem serif without clipping. (2) After a wrong password, the error state (red underline + error icon) persisted even when the user started typing a new password. The error icon ate into the visible text area, causing the `.typedText` overlay to misalign since its `width: calc(100% - 50px)` only accounted for the submit arrow, not the error icon.

**Root Cause:** (1) The 360px cap on `.inputArea` and `.passwordInput` was set before the password changed to a longer phrase. The card's 380px was also tight. (2) The `onChange` handler only called `setPassword` without clearing the error. The error persisted until the next form submission.

**Resolution:** (1) Removed `max-width: 360px` from `.inputArea` and `.passwordInput`, replacing with `width: 100%`. Raised `.card` from `max-width: 380px` to `440px` to accommodate the full password text. On the canonical 1120px inner (`.canvasPane` at 55% = 616px, `.formPane` gets 504px, minus padding = 440px available), the card fills exactly. On mobile (<768px), the card is `100vw - 96px`, scaling naturally. (2) Added `if (error) setError("")` inside the `onChange` handler so the error clears the moment the user starts typing. This eliminates the error-icon-while-typing scenario entirely, making the `.placeholderTruncated` class dead code (removed). Updated `.inner` comment to reflect the new 440px card width.

**Files modified:** `src/app/(frontend)/for/[company]/LoginClient.tsx`, `src/app/(frontend)/for/[company]/login.module.scss`

**Principle:** Input field widths for password/passphrase fields must be sized to the longest expected valid input, not to an arbitrary cap. Error states should clear on user re-engagement (typing), not persist until the next explicit submission.
...
**Issue:** On the login page (`/for/[company]`), when the user hits Enter to submit the password, the actual input text (normally invisible via `color: transparent`) briefly appears as gray text. The login form uses a custom text overlay system: the native `<input>` has `color: transparent` and a `.typedText` span renders the visible password. During form submission, `isPending` sets `disabled` on the input, and the DS Input component's disabled styling overrides the transparent color.

**Root Cause:** CSS specificity conflict. The page-level rule `.passwordInput input { color: transparent }` compiles to a two-class selector, but the DS Input component's disabled rule `.minimal .inputContainer.disabled .input` compiles to a four-class selector (higher specificity). When `disabled` is set, the component's `color: var(--portfolio-text-neutral-disabled)` wins over the page's `color: transparent`, making the hidden input text visible as gray text behind the overlay.

**Resolution:** Added `!important` to `color: transparent` in `.passwordInput input` in `login.module.scss` to ensure it always wins regardless of the DS component's internal specificity. Also added `caret-color: transparent` on `&:disabled` since there's no reason to show a blinking cursor in a disabled input. The `!important` is justified here because the page is intentionally overriding a component's internal implementation detail (the input must never show its own text, regardless of state), and the alternative (threading a custom class onto the native input via a new Input prop) would be a disproportionate API change for a single-use case.

**Files modified:** `src/app/(frontend)/for/[company]/login.module.scss`

**Principle:** When a page-level override depends on a component's internal styling staying inert, the override must account for all state variants of the component, not just the default state. Disabled/readonly/error states often carry color overrides that can pierce a `color: transparent` hack.
...
**Issue:** The `altPasswords` feature was added as a quick fix (comma-separated text field) and only wired into `actions.ts`. The admin dashboard, data layer types, and skill docs were unaware of it. The global fallback password ("glad you are here") only worked for unknown companies, not for known ones with their own passwords. The case study page lacked a `!== "welcome"` guard that the home page already had.

**Root Cause:** Rapid incident response (user accidentally put "chalk" on resume) led to a minimal fix without propagating the change across all layers.

**Resolution:** (1) Migrated `altPasswords` from comma-separated `text` to Payload `array` of `{ value }` objects in `Companies.ts`. (2) Created `companies_alt_passwords` table in `push-schema.ts` (mirrors `companies_case_study_notes` pattern), idempotently dropped old `alt_passwords` varchar column. (3) Updated `CompanyRecord.altPasswords` type from `string` to `string[]` in `company-data.ts` with array mapper. (4) Simplified `actions.ts` validation: removed `.split(",")` parsing, added `SITE_PASSWORD` and hardcoded "glad you are here" as universal fallbacks for all companies. (5) Added `!== "welcome"` guard to `work/[slug]/page.tsx` to match the home page's defense-in-depth. (6) Updated `CompanyDashboard.tsx` with `altPasswords` in types, fetch, submit, edit, and form UI (textarea, one per line). (7) Updated `password-gate/SKILL.md` and `personalize/SKILL.md` with new field, fallback behavior, and invariants. Also fixed `greeting` default mismatch (`'Welcome'` -> `'Welcome.'`) in `push-schema.ts`.

**Files modified:** `src/collections/Companies.ts`, `src/scripts/push-schema.ts`, `src/lib/company-data.ts`, `src/app/(frontend)/for/[company]/actions.ts`, `src/app/(frontend)/(site)/work/[slug]/page.tsx`, `src/components/admin/CompanyDashboard.tsx`, `.cursor/skills/password-gate/SKILL.md`, `.cursor/skills/personalize/SKILL.md`

**Principle:** Access-gated features that touch multiple layers (schema, data, validation, admin UI, docs) need a full propagation pass, not incremental patches. A quick fix that only wires one layer creates invisible gaps that compound.
...
**Issue:** User requested boot up. `curl` to `http://127.0.0.1:4000/` and `:4001/` timed out despite `lsof` showing `next-server` listeners; `:4002/` initially returned 200 then later timed out the same way.

**Root Cause:** Long-lived Next.js dev processes entered a state where the socket accepted connections but the HTTP stack never completed responses (same failure mode as ENG-116 / EAP-068).

**Resolution:** Identified PIDs via `lsof -i :PORT -sTCP:LISTEN`, `kill -9` on each zombie, `rm -rf` `.next`, `playground/.next`, and `ascii-tool/.next`, restarted `npm run dev`, `npm run playground`, and `npm run ascii-tool`. Verified `curl -L` on `/` → 200, `/for/welcome?preview=true` → 200, playground `/` → 200, ASCII `/` → 200. Updated `docs/port-registry.md` (PIDs 92481 / 92517 / 94130).

**Principle:** Port registry "running" + `lsof` is not sufficient; boot-up health checks must remain `curl` with a finite timeout.
...
**Issue:** User deleted an empty image upload section ("Before and after: the same design task handled in session 1 vs. session 40") from the Elan case study in the CMS multiple times, but it reappeared every time a deploy/checkpoint ran.

**Root Cause:** `src/app/(frontend)/api/update-elan/route.ts` contained `imagePlaceholders: ['Before and after: ...']` in the first content block. The seeding endpoint overwrites the entire `content` field on every call, so any CMS-side deletion is reversed the next time the endpoint fires.

**Resolution:** Removed `imagePlaceholders` from the first content block in the seeding route. Called `POST /api/update-elan` to push the change to the CMS. Verified the placeholder text is absent from the rendered page.

**Principle extracted -> Content seeding routes are the source of truth for CMS structure. Deleting content in the CMS admin is ephemeral if the seeding route still defines it. To permanently remove CMS content, remove it from the seeding route first, then call the endpoint.**
...
**Root Cause:** The login page server component (`src/app/(frontend)/for/[company]/page.tsx`) checked for *any* existing session cookie and redirected to `/` if one existed, regardless of whether it matched the company in the URL. When a user had a prior "unknown" session (from visiting the site without credentials), navigating to `/for/cognition` never showed the login form - it bounced straight to `/` carrying the "unknown" cookie. The home page's personalization logic correctly skips "unknown" sessions, so no badge appeared.
**Resolution:** Changed the redirect guard from `if (existingSession)` to `if (existingSession && existingSession === company)`. Now the login form only auto-skips when the session already matches the target company. A mismatched or "unknown" session shows the form, allowing the visitor to authenticate with the correct company credentials.
**Principle extracted -> `engineering-anti-patterns.md` EAP-118: Session-existence check without identity match on re-auth pages**
...
**Root Cause:** The personalization data flow from session cookie to home page did not exist. The home page server component had no company session awareness. Separately, the dead essay badge logic (`SHOW_ESSAY_FORMAT_BADGE_ON_HOME = false`, `Divider`, `isEssay`/`prevIsEssay`/`needsDivider`, `contentFormat` in the home type) was occupying the badge slot and adding unused code.

**Resolution:** (1) `page.tsx`: added `getCompanyFromSession` + `getCompanyBySlug` in a separate `try/catch` (isolated from the Payload projects fetch). Guards: skip if slug is falsy or `"unknown"`, filter `caseStudyNotes` to entries with non-empty `note`, pass `personalization: { name, slugs }` to `HomeClient`. Removed `contentFormat` from the case study type and map. (2) `HomeClient.tsx`: removed `SHOW_ESSAY_FORMAT_BADGE_ON_HOME`, `isEssay`/`prevIsEssay`/`needsDivider`, `Fragment` wrapper, `Divider` import, `contentFormat` from `CaseStudy` type. Added `personalization` prop. Badge renders as `<Badge appearance="neutral" emphasis="minimal" size="sm" shape="squared">for {name}</Badge>` before the headline, same visual slot as the old essay badge. (3) `page.module.scss`: replaced `.essayBadge` and `.sectionDivider` with `.personalizationBadge`.

**Principle:** Session-derived personalization data should flow through its own error boundary so a company DB failure never degrades the primary content. Badge label "for {Company}" echoes the login URL convention and reads as a quiet aside rather than a CRM injection.

**Cross-category note:** Also documented as FB-176 (design) for the badge label decision and visual slot repurposing.
...
**Root Cause:** The previous Drive file id was hardcoded in `FOOTER_LINK_DEFAULTS_BY_LABEL` (`layout.tsx`) and in both seed implementations (`src/scripts/seed.ts`, `src/app/(frontend)/api/seed/route.ts`).

**Resolution:** Replaced the file id in all three locations. Empty `socialLinks[].href` in CMS still resolves via `resolveFooterSocialLink`; a non-empty stored href continues to win until the author updates Payload or re-runs a seed that writes `site-config`.
...
**Root Cause:** Two compounding factors: (1) The link popover's auto-focus `useEffect` used `requestAnimationFrame(() => linkInputRef.current?.focus())` which fired during Lexical's selection reconciliation cycle. Calling `.focus()` on a non-Lexical element while Lexical 0.41's frozen committed `Point` objects are being reconciled by React 19 StrictMode triggers the read-only property error. (2) The dev server was not restarted after the structural changes (ENG-198/199 removed `ClickableLinkPlugin` and added new hooks/effects), so Webpack HMR may have partially delivered the changes — recurrence of ENG-189/EAP-115 pattern.

**Resolution:** (a) Replaced `requestAnimationFrame(() => focus())` with `setTimeout(() => focus(), 0)` to defer the focus call past Lexical's current reconciliation microtask + rAF cycle. `setTimeout(fn, 0)` is guaranteed to run in a subsequent event-loop task, after all pending microtasks and rAF callbacks have completed. (b) Used a `prevVisibleRef` guard on the visibility cleanup effect so it only fires on `true → false` transitions, not on mount. (c) Clean dev-server restart (`kill -9`, `rm -rf .next`, `npm run dev`) to ensure all structural hook changes are freshly compiled and delivered — per EAP-115 this is mandatory for changes that alter React hook shape.

**Principle extracted -> `engineering.md` §14: Never call `.focus()` on a non-Lexical DOM element inside `requestAnimationFrame` from a Lexical plugin or toolbar — use `setTimeout(fn, 0)` to defer past the reconciliation cycle.**
...
**Root Cause:** `ClickableLinkPlugin` from `@lexical/react` was mounted in `LexicalBlockEditor.tsx`. This plugin intercepts clicks on link nodes to navigate to them (open the URL) - behavior appropriate for a read-only document viewer but wrong for an editing context. It prevented users from placing their cursor inside linked text to edit the display text.

**Resolution:** Removed `ClickableLinkPlugin` entirely from `LexicalBlockEditor.tsx`. In an editing context, clicking a link should place the cursor for text editing, not navigate. The "open link in new tab" functionality is already available via the link popover (ENG-198). Added `cursor: text` to both the `.lexContentEditable a` and `.lexLink` CSS rules to reinforce that links are editable text in the editor.

**Principle extracted -> `engineering.md` §14: In editing contexts, never use `ClickableLinkPlugin` — it converts the editor from an editing surface to a reading surface for links. Link navigation belongs in the toolbar popover, not on click.**
...
**Root Cause:** The `toggleLink` callback in `LexicalToolbar.tsx` had a binary toggle: `isLink ? dispatch(null) : prompt('Enter URL')`. The destructive action (unlinking) had no intermediate state, no URL preview, and no confirmation. The constructive action (creating a link) used `window.prompt`, which is a poor UX primitive.

**Resolution:** Replaced the binary toggle with a link popover system. When the selection is on a link, clicking the link button now opens a compact popover showing: (1) the current URL as a truncated monospaced link, (2) an edit button to switch to an input field, (3) an open-in-new-tab button to verify the URL, and (4) a remove-link button with a danger hover state. When the selection is NOT on a link, clicking the link button opens the popover in edit mode directly with an empty URL input. The `window.prompt` is eliminated. The popover auto-closes when the toolbar hides (selection collapses or changes). Added `findLinkNode()` helper that reads the `LinkNode.getURL()` to extract the URL from the selection. New SCSS: `.lexLinkPopover`, `.lexLinkPreviewRow`, `.lexLinkEditRow`, `.lexLinkInput`, `.lexLinkActionBtn`, `.lexLinkConfirmBtn` in `inline-edit.module.scss`.

**Principle extracted -> `engineering.md` §14: Destructive actions on inline-edit toolbar must always show a preview of what will be destroyed and offer an explicit choice, never fire immediately on a toggle click.**

**Cross-category note:** Also documented as FB-174 (design — toolbar affordance gap: toggle button conflating "inspect" and "destroy" actions).
...
**Root Cause:** Same serialization mismatch discovered in ENG-195 but on the *editing* path, not the seeding path. The `LexicalBlockEditor` registers `@lexical/link`'s `LinkNode` and the toolbar dispatches `TOGGLE_LINK_COMMAND` from `@lexical/link`, which creates nodes serializing as `{ type: "link", url: "https://..." }`. The `save()` function in `LexicalBlockEditor.tsx` calls `state.toJSON()` and writes this raw JSON to the CMS. On the visitor side, `safeConvertToHtml()` uses Payload's `defaultHTMLConverters`, whose `LinkHTMLConverter` reads `node.fields.url` — which is `undefined`, producing `<a href="">`.

**Resolution:** Extracted `normalizePayloadLinks` from `content-helpers.ts` into a standalone pure-function module `src/lib/normalize-lexical-links.ts` (no Lexical runtime dependency, safe for client bundles). Imported it into `LexicalBlockEditor.tsx` and applied it in the `save()` function: `const json = normalizePayloadLinks(state.toJSON())`. This transforms any `@lexical/link` `LinkNode` (`{ type: "link", url }`) into Payload's format (`{ type: "link", fields: { url, linkType: "custom", newTab: true } }`) at save time. The in-editor experience is unchanged (user still sees the link, can toggle it); the transform only affects the JSON written to the CMS. Re-ran the Élan seeder to apply the previously-pending link on "Élan" in the scope statement.

**Principle:** When two libraries share a node type name but disagree on serialization format, the bridge transform must live on EVERY path that writes to the shared store (CMS), not just the first one discovered. ENG-195 fixed the seeding path; this fixes the editing path. A single `normalizePayloadLinks` module now serves both.
...
**Issue:** Adding a markdown link (`[Élan](url)`) to the Élan scope statement rendered an `<a>` with an empty `href`. Discovered during plan audit before implementation, never shipped broken.

**Root cause:** `markdownToLexical()` in `src/lib/content-helpers.ts` registers `@lexical/link`'s `LinkNode`, which serializes as `{ type: "link", url: "..." }`. The rendering pipeline (`safeConvertToHtml` in `page.tsx`) uses Payload's `convertLexicalToHTML` with `defaultHTMLConverters`, whose `LinkHTMLConverter` reads `node.fields.url` (Payload's own `LinkNode` shape: `{ type: "link", fields: { url, linkType, newTab } }`). The two libraries both export a class named `LinkNode` with incompatible serialization formats. No existing content seeding route had ever used markdown link syntax, so the incompatibility was latent.

**Resolution:** Added `normalizePayloadLinks()` post-conversion transform inside `markdownToLexical()`. Walks the serialized JSON, finds any node with `type: "link"` that has a top-level `url` instead of `fields.url`, reshapes it to Payload's format with `linkType: "custom"` and `newTab: true`. Idempotent (skips nodes already in Payload format). The transform runs on every `markdownToLexical` call but is a no-op when no link nodes exist. Promoted EAP-119.

**Principle:** When two libraries export identically-named classes with different serialization contracts (here, `@lexical/link` `LinkNode` vs `@payloadcms/richtext-lexical` `LinkNode`), a boundary transform is required at the serialization boundary. The type name match masks the format mismatch. Test the full data path (write -> store -> read -> render) before assuming format compatibility.
...
**Issue:** Console `TypeError: Cannot assign to read only property 'key' of object '#<Point>'` (Next.js 16.2.1 Webpack) on the case-study inline-edit stack, same family as ENG-188 / ENG-189.

**Root Cause:** ENG-188 removed command-registration churn and StrictMode `initialConfig` aliasing, but `useBlockKeyboardNav` still moved focus with `requestAnimationFrame` + `window.getSelection().selectAllChildren` / `collapseToEnd()` while the previous `LexicalComposer` could still be reconciling DOM selection against a **committed** `EditorState`. Lexical dev mode freezes `RangeSelection.anchor` and `.focus` on commit (`Object.freeze` in `Lexical.dev.mjs` ~8517-8525). Overlapping native selection writes with that window produces writes to frozen `Point` objects from inside Lexical's selection pipeline.

**Resolution:** In `src/components/inline-edit/useBlockKeyboardNav.ts`, defer one tick with `queueMicrotask`, call `getNearestEditorFromDOMNode(document.activeElement)?.blur()` from `lexical` (public API), then `requestAnimationFrame` to `focus()` the target wrapper and run the existing end-of-block native selection placement. Ordering: microtask clears Lexical's stack for the key command, blur drops the outgoing editor cleanly, rAF focuses the incoming surface.

**Principle extracted:** `docs/engineering.md` §14.9.3 (programmatic multi-editor focus must blur Lexical before native `Selection` APIs).

**Cross-category note:** None (engineering-only).

**Files touched:** `src/components/inline-edit/useBlockKeyboardNav.ts`, `docs/engineering.md`, `docs/engineering-anti-patterns.md`, `docs/engineering-feedback-log.md`.
...
**Root cause:** Payload CMS stores link nodes as `{ type: "link", fields: { url, linkType, newTab } }` (Payload's `LinkNode` serialization). Standard `@lexical/link`'s `LinkNode.updateFromJSON` reads `serializedNode.url` at the top level. When the CMS data is passed directly to `LexicalComposer`'s `editorState`, `url` is `undefined`, which gets set as `__url`. When the `MarkdownShortcutsPlugin` later processes nodes and calls `linkNode.getURL().match(...)`, it crashes on `undefined.match()`. The project already had `normalizePayloadLinks()` (Lexical -> Payload format) for the save path, but lacked the reverse transform for the load path.

**Resolution:** Created `denormalizePayloadLinks()` in `src/lib/normalize-lexical-links.ts` — the inverse of `normalizePayloadLinks()`. It reshapes `{ fields: { url } }` back to `{ url, target, rel }` before the data reaches Lexical's deserializer. Applied in two locations:
1. `LexicalBlockEditor.tsx` — wraps `initialState` with `denormalizePayloadLinks()` before `JSON.stringify` for `editorState`
2. `content-helpers.ts` — wraps CMS data in `lexicalToMarkdown()` before `parseEditorState` (was silently swallowed by try-catch)
...
**Root cause:** `minHeight` sets a floor, not a ceiling. The grid can still grow beyond the measured value if any column's content pushes past it. The intent was to lock the height at the tallest single-card-expanded state, which requires `height` (not `minHeight`) combined with `overflowY: hidden` to prevent content from expanding the element.

**Resolution:** Renamed `gridMinHeight` → `gridFixedHeight`. Changed the inline style from `{ minHeight: gridFixedHeight }` to `{ height: gridFixedHeight, overflowY: 'hidden' }`. Also used `overflowY` rather than `overflow` to preserve the CSS-defined `overflow-x: auto` on the grid.

**Files modified:** `src/components/elan-visuals/SkillMap.tsx`

**Principle:** "Fixed height" = `height` + `overflowY: hidden`. `minHeight` is a floor constraint, not a lock. Whenever the goal is to prevent layout shifts from content changes, use `height` with explicit overflow handling, not `minHeight`.
...
**Root cause:** Two distinct bugs. (1) The tooltip position was derived directly from the live `tooltip` state with a `?? 0` fallback, so position snapped to origin the moment tooltip became null. (2) There was no scale or directional animation — dismiss was only opacity + height collapse.

**Resolution:** Split the tooltip into two elements: `.treemapTooltipAnchor` (handles `position: absolute` + `transform: translate(-50%, -100%)`) and `.treemapTooltip` (handles visual styling + scale animation). A `lastTooltipPosRef` ref holds the last known position; the anchor reads from it when `tooltip` is null, keeping the dismiss animation stationary. On container `onMouseLeave`, the exit position is measured and converted to the nearest corner quadrant (`0%`/`100%` for x and y), which is set as the CSS `transform-origin` on the visual element. The tooltip then `scale(0)` transitions toward that corner. Added `transform` to the SCSS transition list. Also removed `setTooltip(null)` from tile `onMouseLeave` (only the container dismisses) to prevent scale flicker during tile-to-tile navigation.
...
### ENG-255: Link labels not rendering — stale closure in linkCanvasObject callback

**Issue:** Link labels (edge type + confidence text) were implemented but never appeared on screen when hovering nodes in Mesh view.

**Root cause:** `paintLinkLabel` used `hoveredNeighborLinks` as a `useCallback` closure variable and dependency. When hover state changed, the callback was recreated (new function ref). `react-kapsule` (the React wrapper for the force-graph kapsule) detected the prop change via identity comparison (`prevPropsRef.current[p] !== props[p]`) and called `comp.linkCanvasObject(newFn)`. However, the kapsule's `onChange: notifyRedraw` schedules a redraw asynchronously. During the repaint, the old callback (captured in the kapsule's state before the update propagated) could still be executing with stale `hoveredNeighborLinks === null`, causing the early-return guard to always exit. Additionally, inline arrow functions for `linkCanvasObjectMode`, `nodeCanvasObjectMode`, and `linkLabel` created new function refs every render, causing unnecessary kapsule updates that could interfere with prop propagation ordering.

**Resolution:**
1. Moved `hoveredNeighborLinks` to a ref (`hoveredNeighborLinksRef`), following the same pattern already used for `activatedPathRef` and `previewPathRef`. The stable callback reads current state via the ref without needing recreation.
2. Hoisted inline arrow functions (`() => "after"`, `() => "replace"`, `() => ""`) to module-level constants (`linkCanvasAfterMode`, `nodeCanvasReplaceMode`, `emptyLinkLabel`) so they maintain stable identity across renders. Note: `accessorFn` treats raw strings as property name lookups (`obj[str]`), so these MUST be functions, not string literals.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** Canvas callbacks passed to kapsule-based libraries (via `react-kapsule`) should use refs for rapidly-changing state (hover, selection) rather than `useCallback` dependencies. The kapsule's async redraw scheduling means prop updates and paint cycles are not synchronous — a callback that closes over state may execute with stale values. The ref pattern (`useRef` + assign `.current` on every render) ensures the paint callback always reads the latest state. This is the same pattern the codebase already uses for `activatedPathRef`/`previewPathRef` (Signal view particle emission).

---

### ENG-257: Pathway hover emphasis — two-layer core + glow rendering

**Date:** 2026-04-26
**Category:** ForceGraph / rendering
**Tags:** `EAP-042`

**Issue:** Pathway view emphasized links rendered as single semi-transparent lines. User wanted solid arrows with translucent glow for thickness, not uniform opacity.

**Root cause:** `linkColorAccessor` returned `rgba()` colors with alpha for emphasis. The library draws only one stroke per link, so there was no way to composite a solid core over a translucent glow using just color/width props.

**Resolution:** Split emphasis into two layers:
1. **Library-drawn glow**: `TERRA_LINK_EMPHASIS_GLOW` / `LUMEN_LINK_EMPHASIS_GLOW` (low alpha ~0.18/0.15) at `base * 2.5` width
2. **Custom core**: `paintLinkOverlay` callback (`linkCanvasObject` in "after" mode) draws a 0.6px solid `rgb()` stroke on top using the link's `__controlPoints` for curved paths or straight `lineTo` for uncurved

Arrow colors (`TERRA_ARROW`, `LUMEN_ARROW`) changed from `rgba()` to `rgb()` for full solidity.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When a visualization library limits stroke to a single layer, use the "after"-mode canvas callback to composite a second stroke. Draw the glow (wider, translucent) as the library's default, then the core (thinner, solid) via the callback.

---

### ENG-256: Bidirectional arrow curve direction swap for outward-facing arrowheads

**Issue:** In Pathway view, bidirectional link arrows had their arrowheads pointing inward (toward each other), creating visual clash.

**Root cause:** Curve direction assignment: forward = `+1`, reverse = `-1`. The library places arrowheads at the target end of each link. With this curving, both arrowheads ended up on the inner edges of the curves.

**Resolution:** Swapped values: forward = `-1`, reverse = `+1`. Arrowheads now sit on the outer edges. Also updated the link label dedup guard from `=== -1` to `=== 1` to continue skipping the reverse (swapped source/target) copy.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When expanding bidirectional links into paired curved arrows, the curve direction determines where arrowheads land relative to each other. Test both orientations visually - the mathematical sign doesn't intuitively map to "inward" or "outward" without seeing the result.

---

### ENG-260: Hollow sink node interior masking via background fill

**Issue:** `force-graph` draws link lines and particles through node centers. Sink nodes rendered as hollow rings had visible link geometry passing through their empty centers.

**Root cause:** The library's render pipeline is: links → arrows → particles → nodes. By the time `nodeCanvasObject` runs, all link/particle geometry is already on the canvas. There is no built-in prop to stop links at node boundaries when using custom node rendering in "replace" mode.

**Resolution:** Added `bgColorRef` (a `useRef<string>`) populated by a `useEffect` that walks up the DOM from `containerRef` to find the first non-transparent computed `backgroundColor`. In `paintNode`, for sink nodes, a solid circle filled with `bgColorRef.current` is drawn first (masking underlying link/particle geometry), then the hollow ring is stroked on top. Falls back to `isDark ? "#1e1e1e" : "#ffffff"` if no opaque ancestor is found.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When a canvas library draws link geometry center-to-center and provides no border-stop API, use the render order (links before nodes) as leverage: fill the node area with the container's background color to mask what's underneath, then draw the node shape on top. A runtime-resolved background color (via `getComputedStyle` DOM walk) makes this approach robust across themes and container contexts.

**Cross-category note:** Also documented as FB-239 (design).

---

### ENG-261: Unified two-layer link emphasis rendering across all ForceGraph views

**Issue:** Hover emphasis on connected links was visually inconsistent. Pathway view used a two-layer glow+core pattern while Mesh and Signal used a single-layer color change.

**Root cause:** `paintLinkOverlay` gated core stroke rendering behind `if (isPathway && isConnected)`. Mesh and Signal `linkColorAccessor` returned a solid emphasis color (`meshLinkEmphasis`) at `LINK_WIDTH_EMPHASIS` width - no glow/core separation.

**Resolution:**
1. Replaced single-value `LINK_COLOR_EMPHASIS` / `LINK_WIDTH_EMPHASIS` with two-layer constants: `LINK_EMPHASIS_WIDTH` (2.5, glow) and `LINK_EMPHASIS_CORE_WIDTH` (0.6, core).
2. Added theme-aware emphasis colors: `meshEmphasisGlow` (translucent) for the library's line, `meshEmphasisCore` (solid) for the overlay.
3. Updated `linkColorAccessor` to return `meshEmphasisGlow` for connected links in Mesh and Signal views.
4. Updated `linkWidthAccessor` to return `LINK_EMPHASIS_WIDTH` for connected links in Mesh and Signal views.
5. Generalized `paintLinkOverlay` core stroke from `if (isPathway && isConnected)` to `if (isConnected)`, with Pathway using semantic Terra/Lumen cores and Mesh/Signal using neutral `meshEmphasisCore`.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When a visualization has multiple view modes that share the same hover interaction model, the visual emphasis pattern (shape, layering) must be identical across modes. Only the color family should differ to match each mode's semantic palette. Inconsistent emphasis patterns break the user's spatial mental model when switching views.

**Cross-category note:** Also documented as FB-240 (design).

---

### ENG-262: ForceGraph auto-fit on layout completion

**Issue:** Force-directed graph rendered at default simulation scale, leaving large empty margins in the container.

**Root cause:** `handleEngineStop` pinned nodes but never adjusted the viewport. The d3-force simulation's default charge/center forces produce a cluster sized independently of the canvas dimensions.

**Resolution:** Added `graphRef.current.zoomToFit(400, 40)` in `handleEngineStop`, guarded by `initialFitDoneRef` (fires once per dataset). A `useEffect` resets the flag when `graphData` changes. The 400ms duration provides a smooth settle animation; 40px padding prevents nodes from touching the canvas edge.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** Force-directed layouts must always auto-fit after the simulation converges. The d3 simulation has no knowledge of the canvas dimensions - it only knows about force parameters. The rendering layer is responsible for mapping the simulation's coordinate space to fill the viewport.

**Cross-category note:** Also documented as FB-241 (design).

---

### ENG-263: Signal hover emphasis color mismatch + glow/core ratio tuning

**Issue:** (1) Signal view hover emphasis used neutral gray (`meshEmphasisGlow`/`meshEmphasisCore`) on paths that displayed Terra/Lumen particles, creating color pollution. (2) Glow width (2.5) was too close to core width (0.6) for the two-layer effect to be perceptible in Mesh and Signal views.

**Root cause:** When unifying hover emphasis across views (ENG-261), Signal was grouped with Mesh into the neutral-color branch. Signal should have been grouped with Pathway since it already uses semantic colors for particles and active paths.

**Resolution:**
1. `linkColorAccessor`: Signal `onActive && connected` now returns `TERRA/LUMEN_LINK_EMPHASIS_GLOW` (matching particle color family).
2. `paintLinkOverlay`: Core color branch changed from `if (isPathway)` to `if (isPathway || isSignal)` for semantic colors. Added `isSignal` to dependency array.
3. Constants: `LINK_EMPHASIS_WIDTH` 2.5 → 4.0, `LINK_EMPHASIS_CORE_WIDTH` 0.6 → 0.5, `meshEmphasisGlow` opacity reduced (0.18/0.15 → 0.12/0.10).

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When unifying visual patterns across view modes, color family assignment must follow the mode's existing semantic palette, not a blanket "everything gets neutral." Signal and Pathway share semantic coloring; Mesh is the only neutral view. The glow-to-core width ratio needs at least ~8:1 for the two-layer structure to read as distinct layers rather than a single thick line.

**Cross-category note:** Also documented as FB-242 (design).

---

### ENG-264: Tooltip degree stats simplified, 2nd-degree BFS removed

**Issue:** `hoveredDegreeStats` memo computed a 2nd-degree neighbor set via BFS that was no longer displayed. The tooltip JSX used a two-row layout with separate `tooltipDegreeRow` and `tooltipDegreeLabel` classes that were replaced by a single-row compact format.

**Root cause:** Information architecture refinement - the 2nd-degree metric was too abstract for a glanceable tooltip. The BFS computation was dead weight after the display decision.

**Resolution:**
1. `hoveredDegreeStats` memo: removed `secondSet` BFS loop and `second` field. Return type changed to `{ total, inCount, outCount }`.
2. Tooltip JSX: replaced two `tooltipDegreeRow` spans with single row: `{total} ({inCount} in / {outCount} out)`. New `tooltipDegreeSplit` class for the parenthetical.
3. SCSS: removed `.tooltipDegreeRow` and `.tooltipDegreeLabel`. Added `.tooltipDegreeSplit` (mono font, 3xs, 0.45 opacity). Changed `.tooltipDegrees` from `column` gap layout to `baseline`-aligned flex row.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`, `src/components/ui/ForceGraph/force-graph.module.scss`

**Principle:** When a computed value is no longer consumed by any UI path, remove the computation immediately. Dead BFS traversals over adjacency maps are O(V+E) per hover and accumulate silently.

**Cross-category note:** Also documented as FB-243 (design).

---

### ENG-265: Organic grow-in animation for progressive disclosure nodes

**Issue:** Progressive disclosure used a linear opacity ramp (`tierAlpha`). Nodes appeared at full size with no spatial animation, feeling mechanical.

**Root cause:** The original `tierAlpha` was a simple linear interpolation: `(scale - threshold) / TIER_RAMP`. No easing curve was applied, and the node radius was always at 100% regardless of disclosure progress.

**Resolution:**
1. Added `sampleCubicBezier` (Newton's method, 8 iterations) to evaluate arbitrary cubic-bezier curves at a given progress value. This is a general-purpose utility used by both `tierAlpha` and `tierScale`.
2. `tierAlpha` now passes the linear progress through the expressive easing curve (0.4, 0.14, 0.3, 1) from `src/lib/motion.ts`, producing eased opacity.
3. New `tierScale` function returns a radius multiplier (0 → ~1.12 → 1.0). Uses a sine-damped overshoot: `eased + sin(eased * PI) * 0.12 * (1 - eased^2)`. The sine bell peaks mid-animation; quadratic damping shifts the effective peak to ~60-70% progress and decays to 0 at completion.
4. `paintNode` applies `tierScale` to the node radius. `nodePointerAreaPaint` also scales the hit area to match.
5. Constants: `EASE_P1X/P1Y/P2X/P2Y` mirror the motion token; `SCALE_OVERSHOOT = 0.12`.

**Files modified:** `src/components/ui/ForceGraph/ForceGraph.tsx`

**Principle:** When adding easing to canvas-rendered animations (no CSS/Framer available), implement cubic-bezier evaluation inline rather than adding a runtime dependency. The Newton's method approximation converges in 4-6 iterations for typical UI curves and adds negligible per-frame cost. The easing constants should reference the motion token system even if they can't import the SCSS values directly.
