...
const CONTENT_BLOCKS = createCaseStudyBlocks(
  [
    ...
    },
    {
      ...
      bodyMarkdown:
        "So I built both. A regex extractor lifted implicit cross-references between rules into typed edges: six relationship types, each with a confidence score. An audit enforcer validates every new entry against the existing graph at write time. Contradictions surface before they ship. The result is ~995 nodes connected by ~4,892 edges. The graph below is live.\n\nAlongside the graph, I designed a 12-task eval corpus pulled from real feedback entries: four design, four engineering, four content. Each has a gold anti-pattern, at least one plausible foil, and a known confound profile. Bare baseline (no skills, no routing, no graph) versus the full system. LLM-as-judge scores each run.\n\nThe eval is running now. The next deliverable is a number, not a feature.",
    },
  ],
  {
