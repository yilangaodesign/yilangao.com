/**
 * @archived
 * @origin homepage-v1 — src/app/(frontend)/(site)/HomeClient.tsx
 * @date 2026-04-09
 * @reason Archived to preserve the current home page layout before a visual redesign.
 *
 * Client component for the home page. Two-column layout: sticky sidebar
 * (identity, bio, experience, links) on the left, masonry project grid
 * with interleaved testimonials on the right. Includes admin features:
 * drag-to-reorder grid (dnd-kit), inline editing, project edit modal,
 * add project/testimonial cards.
 *
 * To restore: copy back to src/app/(frontend)/(site)/HomeClient.tsx
 * alongside page.tsx and page.module.scss from this same archive folder.
 */
...
import { SiteFooter, siteShellStyles } from "@/components/SiteFooter";
...
function ProjectCard({ project, isAdmin, onEdit, priority }: { project: Project; isAdmin?: boolean; onEdit?: (project: Project) => void; priority?: boolean }) {
...
export default function HomeClient({
  ...
  const content = (
    <main className={styles.page}>
                    ...
                    {col.map((item, i) =>
                      item.type === "project" ? (
                        <StaggerItem key={`p-${(item.data as Project).slug}`} className={(item.data as Project).featured ? styles.gridItemFeatured : styles.gridItem}>
                          <ProjectCard project={item.data as Project} isAdmin={isAdmin} onEdit={openProjectEditor} priority={i === 0 && colIdx < 2} />
                        </StaggerItem>
                      ) : (
      ...
      <SiteFooter siteConfig={{ footerCta: siteConfig.footerCta, email: siteConfig.email }} />
