/**
 * @archived
 * @origin homepage-v1 — src/app/(frontend)/(site)/page.tsx
 * @date 2026-04-09
 * @reason Archived to preserve the current home page layout before a visual redesign.
 *
 * Server component for the home page. Fetches projects, testimonials,
 * teams, social links, and site config from Payload CMS, then passes
 * everything to HomeClient for rendering. The two-column sidebar + masonry
 * grid layout is the defining visual pattern of this version.
 *
 * To restore: copy this file back to src/app/(frontend)/(site)/page.tsx,
 * copy HomeClient.tsx back alongside it, and copy page.module.scss.
 * CMS schema and data are unchanged — this is a view-layer-only archive.
 */
  ...
  try {
    ...
    if (projectsRes.docs.length > 0) {
      projects = projectsRes.docs.map((p) => {
        type MediaRef = { id?: string | number; url?: string; mimeType?: string; width?: number; height?: number; sizes?: { card?: { url?: string }; hero?: { url?: string }; thumbnail?: { url?: string } } } | null | undefined;
        const thumb = (p as Record<string, unknown>).thumbnail as MediaRef;
        const hero = p.heroImage as MediaRef;
        const cover = thumb ?? hero;
        const isVideo = cover?.mimeType?.startsWith('video/');
        const cardUrl = !isVideo ? cover?.sizes?.card?.url : null;
        return {
          id: p.id,
          slug: p.slug,
          title: p.title,
          ...
          coverImage: cardUrl ?? cover?.url ?? null,
